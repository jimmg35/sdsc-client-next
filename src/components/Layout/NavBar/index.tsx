...
const Navbar = () => {
  ...
  const shellClass = `mx-auto border transition-[width,max-width,margin,border-radius,border-color,background-color,box-shadow] ${MORPH} ${
    hasScrolled
      ? 'mt-3 w-[calc(100%_-_1rem)] max-w-6xl rounded-[28px] border-accent-600/20 bg-surface/80 shadow-lift backdrop-blur-xl backdrop-saturate-150 md:rounded-[32px] dark:bg-accent-100/80'
      : 'mt-0 w-full max-w-full rounded-none border-transparent border-b-accent-600/20 bg-surface shadow-none dark:bg-accent-50'
  }`;

  const navClass = `relative mx-auto flex max-w-6xl items-center justify-between gap-4 text-primary-700 transition-[height,padding] ${MORPH} ${
    hasScrolled
      ...
      <div className={shellClass}>
        <nav className={navClass}>
          <Link
            href="/"
            aria-label={t('home')}
            className="shrink-0 rounded-md focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary-400"
          >
          ...
          <div className="hidden min-w-0 items-center gap-3 lg:flex">
                ...
                return (
                  <li key={item.key}>
                    <Link
                      ...
                      className={`group relative inline-flex items-center gap-2 whitespace-nowrap rounded-md py-2 text-[0.7rem] font-semibold uppercase transition-colors duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary-400 ${
                        isDense
            ...
            <span aria-hidden className="h-5 w-px bg-accent-600/25" />
            <ThemeToggle />
          </div>

          <button
            type="button"
            className="relative inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-accent-600/25 bg-surface/80 text-primary-600 transition hover:border-primary-300 hover:text-primary-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary-400 dark:bg-accent-200/80 lg:hidden"
        ...
        >
          <div className="min-h-0 overflow-hidden">
            <div
              className={`border-t border-accent-600/20 pt-5 ${
                hasScrolled ? 'mx-3 pb-4' : 'mx-4 pb-6'
              }`}
            >
              <p className="text-[0.65rem] font-semibold uppercase tracking-[0.3em] text-primary-500">
                {t('navigation')}
              </p>

              <ul className="mt-2 divide-y divide-accent-600/15">
              ...
              <div className="mt-4 flex items-center gap-3 border-t border-accent-600/15 pt-4">
                <ThemeToggle variant="mobile" />
                <Link
                  href="/contact"
                  className="inline-flex h-11 flex-1 items-center justify-center rounded-full border border-accent-300 bg-accent-200/90 px-4 text-xs font-semibold uppercase tracking-[0.24em] text-accent-800 transition hover:bg-accent-200"
                  onClick={() => setIsOpen(false)}
                >
                  {t('partnerCta')}
                </Link>
