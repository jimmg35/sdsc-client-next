...
const Members = () => {
  ...
  return (
    <section className="surface-fade relative overflow-hidden px-6 py-12 md:px-16">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(120%_120%_at_80%_-10%,_rgba(206,184,136,0.35),_transparent_65%)]" />
      ...
      <div className="relative flex flex-col items-center gap-6 text-center text-ink-900">
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-8">
          {featuredMembers.map((member) => (
            ...
            >
              ...
              <p className="mt-4 text-xs font-semibold uppercase tracking-[0.24em] text-garnet-600 transition-colors group-hover:text-garnet-700">
                {member.name}
              </p>
              <p className="mt-1 text-[0.7rem] text-ink-500">{member.title}</p>
            </Link>
          ))}
          <Link
            className="group flex h-[140px] w-[140px] flex-col items-center justify-center rounded-full border border-garnet-200/70 bg-white/80 text-xs font-semibold uppercase tracking-[0.24em] text-garnet-600 transition hover:border-garnet-300 hover:bg-gold-100/80 hover:text-garnet-700"
            href="/member"
          >
            Explore All
          </Link>
