'use client';

export default function Events() {
  return (
    <section className="page-shell">
      <div className="mx-auto max-w-5xl px-6 pb-24 pt-36 text-center text-gold-100 md:pt-40">
        <span className="chip-gold">Events</span>
        <h1 className="mt-6 text-4xl font-semibold text-gold-50 text-glow md:text-5xl">
          Upcoming gatherings and live sessions
        </h1>
        <p className="mx-auto mt-5 max-w-2xl text-sm text-gold-200/80 md:text-base">
          We are curating the next wave of SDSC workshops, talks, and studio showcases. Check back soon for registration details and opportunities to connect in person or online.
        </p>
      </div>
    </section>
  );
}
